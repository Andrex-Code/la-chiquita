    ...
    <main style={styles.page}>
      <div style={styles.header}>
        <div>
          ...
          <p>Administra los productos visibles en la página.</p>
        </div>
        <button style={styles.secondaryButton} onClick={signOut}>Cerrar sesión</button>
      </div>
      ...
      <form onSubmit={handleSave} style={styles.card}>
        ...
        <div style={styles.grid}>
          ...
          <input style={styles.input} placeholder="Slug (opcional)" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} />
          ...
          <input style={styles.input} placeholder="URL de imagen" value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} />
        </div>
        ...
        <div style={styles.actions}>
          <button style={styles.primaryButton} type="submit">{isEditing ? 'Actualizar' : 'Crear producto'}</button>
          {isEditing && <button style={styles.secondaryButton} type="button" onClick={() => { setEditingId(null); setForm(emptyProduct); }}>Cancelar</button>}
        </div>
        {message && <p style={styles.message}>{message}</p>}
      </form>
        ...
        {products.length === 0 ? <p>No hay productos todavía.</p> : products.map((product) => (
          <article key={product.id} style={styles.productRow}>
            <div>
              ...
              <div>{product.price == null ? 'Sin precio' : `$${Number(product.price).toLocaleString('es-CO')}`}</div>
              <small>{product.active ? 'Visible' : 'Oculto'} · {product.available ? 'Disponible' : 'No disponible'}</small>
            </div>
            <div style={styles.actions}>
              <button style={styles.secondaryButton} onClick={() => startEdit(product)}>Editar</button>
              <button style={styles.dangerButton} onClick={() => removeProduct(product.id)}>Eliminar</button>
            </div>
          </article>
        ))}
      </section>
...
const styles = {
  center: { minHeight: '80vh', display: 'grid', placeItems: 'center', padding: 24, background: '#f8f3ed' },
  page: { maxWidth: 1100, margin: '0 auto', padding: '110px 24px 48px', background: '#f8f3ed', minHeight: '100vh' },
  header: { display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'center', marginBottom: 24, flexWrap: 'wrap' },
  card: { width: '100%', maxWidth: 900, background: '#fff', borderRadius: 18, padding: 24, boxShadow: '0 10px 30px rgba(75,45,25,.12)', marginBottom: 24 },
  title: { margin: 0, color: '#5d3824' },
  label: { display: 'block', marginTop: 14, marginBottom: 6, fontWeight: 700 },
  input: { width: '100%', padding: '12px 14px', border: '1px solid #d8c8bb', borderRadius: 10, marginBottom: 12, font: 'inherit', boxSizing: 'border-box' },
